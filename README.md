...
- **Guarda de Rotas (Sessão):** Utiliza `sessionStorage` (`isAuthenticated`). Se qualquer usuário tentar acessar o `index.html` diretamente pela URL sem ter feito login no `login.html`, o sistema o redireciona imediatamente para a tela de login.
- **Persistência das Salas:** Utiliza `localStorage` (`campus_rooms_state`). O status de cada sala (livre/ocupada) e o nome da disciplina ficam salvos no navegador para que a informação não se perca ao fechar ou atualizar a página.
- **Encerramento de Sessão (Logout):** Um botão na barra superior limpa a chave do `sessionStorage` e retorna o usuário à tela de login.

---

Você entendeu a estrutura e o funcionamento do projeto? O que achou dessa arquitetura inicial?
